      ...
      if (this.useRouter) {
        return match({ routes:createOptions.routes, location:thing}, function reactRouterMatchHandler(error, redirectLocation, renderProps) {
          if (error) {
            ...
            var err = new Errors.Router500Error(error.message);
            return done(err);
          } else if (redirectLocation) {
            debug('server.js match 302 %s', redirectLocation.pathname + redirectLocation.search);
            var err = new Errors.Router302Error(redirectLocation.pathname + redirectLocation.search);
            return done(err);
          } else if (renderProps) {
            ...
            var err = new Errors.Router404Error();
            return done(err);
          }
        });
      }
