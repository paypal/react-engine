...
exports.create = function create(createOptions) {
  ...
  return function render(thing, options, callback) {
    ...
    try {
      if (this.useRouter) {
        var location = new Location(thing, {});
        // runs the react router that gives the Component to render
        Router.run(routes, location, function onRouterRun(error, initialState, transition) {
          // TODO: must refactor when react-router's v1.X api fixed.
          var appComponent = initialState.branch[0].component,
              stateComponent = initialState.branch[1].component;

          componentInstance = React.createElement(appComponent, data,
            React.createElement(stateComponent, data)
          );
        });
      }
      else {
