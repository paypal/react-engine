  ...
  return function render(thing, options, callback) {
    ...
    try {
      if (this.useRouter) {
        ...
        // normalzie absolute view file path and absolute base directory path
        // to prevent path strings like `/folder1/folder2/../../folder3/exampleFile`
        thing = path.normalize(thing);
        options.settings.views = path.normalize(options.settings.views);

        // derive relative view file path
        data.__meta.view = thing.replace(options.settings.views, '').substring(1);

        // replace backslash with slash to be compatible on Windows
        data.__meta.view = data.__meta.view.replace('\\', '/');
