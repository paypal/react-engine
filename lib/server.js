...
exports.create = function create(createOptions) {
  ...
  return function render(thing, options, callback) {
    ...
    function done(err, html) {
      ...
      console.log('done with %s', html);
      callback(err, html);
    }

    function renderAndDecorate(component, html) {
      ...
      if (this.useRouter) {
        return match({ routes:createOptions.routes, location:thing},
          ...
          if (redirectLocation) {
            ...
            console.log('The redirectLocation is', require('util').inspect(redirectLocation));
            ...
            return done(null, renderAndDecorate(React.createElement(RoutingContext, renderProps), html));
        ...
        return done(null, renderAndDecorate(component(data), html));
      }
    }
    catch (err) {
