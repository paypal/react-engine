...
exports.create = function create(createOptions) {
  ...
  return function render(thing, options, callback) {
    ...
    if (createOptions.routes && createOptions.routesFilePath) {
      // if `routesFilePath` property is provided, in
      // development env the routes are reloaded for every render.
    ...
    try {
      ...
      // post-render hook
      if (createOptions.postRender && typeof createOptions.postRender == 'function') {
        createOptions.postRender(html, data);
      }
