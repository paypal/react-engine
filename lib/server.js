...
exports.create = function create(createOptions) {
  ...
  return function render(thing, options, callback) {
    ...
    if (createOptions.reactRoutes) {
      createOptions.routes = require(createOptions.reactRoutes);
    }
    ...
    var componentInstance, router;

    try {
      if (this.useRouter) {
        createOptions.location = thing;
        router = Router.create(omit(createOptions, 'reactRoutes'));

        // runs the react router that gives the Component to render
        router.run(function(Component) {
