  ...
  return function render(thing, options, callback) {
    ...
    try {
      if (this.useRouter) {
        ...
        Router.run(routes, thing, function onRouterRun(Component, state) {
          // merge params (The dynamic segments of the url) and path from router which are
          // supposed to be injected into our component.
          data.params = merge(data.params || {},state.params);
          data.path = data.path || state.path;
